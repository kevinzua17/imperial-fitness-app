...
class User(Base):
    ...
    service_tier: Mapped[str] = mapped_column(String(30), default="premium", index=True)
    experience_mode: Mapped[str] = mapped_column(String(30), default="premium", index=True)
...
class ClientPortalLink(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    token_hash: Mapped[str] = mapped_column(String(255), unique=True, index=True)
    label: Mapped[str] = mapped_column(String(120), default="Seguimiento Imperial")
    expires_at: Mapped[datetime] = mapped_column(DateTime, index=True)
...
class LiteSession(Base):
    __tablename__ = "lite_sessions"

    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    source_link_id: Mapped[int | None] = mapped_column(ForeignKey("client_portal_links.id"), nullable=True, index=True)
...
class WellnessCheckin(Base):
    ...
    user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), index=True)
    weight: Mapped[float | None] = mapped_column(Float, nullable=True)
    training_sessions: Mapped[int | None] = mapped_column(Integer, nullable=True)
    nutrition_adherence: Mapped[int | None] = mapped_column(Integer, nullable=True)
    ...
    pain_present: Mapped[int] = mapped_column(Integer, default=0, index=True)
    pain_score: Mapped[int | None] = mapped_column(Integer, nullable=True, index=True)
    pain_area: Mapped[str] = mapped_column(String(80), default="")
    pain_trigger: Mapped[str] = mapped_column(String(300), default="")
    notes: Mapped[str] = mapped_column(Text, default="")
    source: Mapped[str] = mapped_column(String(30), default="lite_link", index=True)
...
class PlanPublication(Base):
    ...
    __table_args__ = (UniqueConstraint("user_id", "plan_type", "version", name="uq_plan_publication_version"),)
...
class ClientIntakeSurvey(Base):
    __tablename__ = "client_intake_surveys"
