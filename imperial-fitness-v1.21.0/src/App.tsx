...
const CoachOperationsPanel = lazy(() => import('./components/CoachOperationsPanel').then(module => ({ default: module.CoachOperationsPanel })));
...
export default function App() {
  ...
  const allowedTabsByRole: Record<ClientProfile['role'], string[]> = {
    admin: ['dashboard', 'profile', 'clients', 'coach_ops', 'personal_plan', 'timer', 'exercises', 'imperial_path', 'social', 'chat', 'history', 'photos', 'body_metrics', 'challenges', 'membership', 'recovery', 'sync', 'specialist_assistant', 'user_management', 'implementation', 'finance'],
    trainer: ['dashboard', 'profile', 'clients', 'coach_ops', 'personal_plan', 'timer', 'exercises', 'imperial_path', 'social', 'chat', 'history', 'photos', 'body_metrics', 'challenges', 'specialist_assistant'],
  ...
  const isTabAllowed = (tab: string, role: ClientProfile['role']) => {
    ...
    if (role === 'client' && currentUser?.experienceMode === 'lite') {
      return ['dashboard', 'profile', 'personal_plan', 'history', 'body_metrics', 'chat', 'membership'].includes(tab);
    }
  ...
  return (
    <div className="min-h-screen bg-black text-neutral-100 flex flex-col font-sans selection:bg-red-600 selection:text-white">
      ...
      <main className="flex-1 pb-16">
        ...
        {currentUser.role !== 'client' && activeTab === 'coach_ops' && (
          <CoachOperationsPanel users={users} />
        )}
