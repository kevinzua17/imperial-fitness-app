...
import { listProgressPhotosFromApi, uploadProgressPhotoToApi } from '../services/progressService';
...
export const ProgressPhotosView: React.FC<ProgressPhotosViewProps> = ({ currentUser, users, onAddPhoto }) => {
  ...
  const [weight, setWeight] = useState(activeClientObj?.weight || 65);
  const [bodyFat, setBodyFat] = useState(activeClientObj?.bodyFat || 20);
  ...
  useEffect(() => {
    ...
    async function loadPhotos() {
      try {
        const photos = await listProgressPhotosFromApi(targetClientId);
        if (!cancelled) setPhotosList(photos);
      } catch {
        if (!cancelled) setPhotosList(activeClientObj?.progressPhotos || []);
      }
    }
    loadPhotos();
    return () => {
  ...
  }, [targetClientId, activeClientObj?.progressPhotos]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setMessage('Foto subida al servidor y guardada correctamente.');
    } catch {
      setMessage('No se pudo subir la imagen. Revisa que el backend esté activo y que el archivo sea válido.');
    } finally {
  ...
  const handleClientChange = (clientId: string) => {
    ...
    if (found) {
      setWeight(found.weight || 65);
      setBodyFat(found.bodyFat || 20);
    }
    setSelectedFile(null);
    setMessage('');
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-neutral-950 via-neutral-900 to-neutral-950 p-6 rounded-2xl border border-neutral-800">
        <div>
          ...
          <p className="text-neutral-400 text-sm mt-1 max-w-xl font-light">
            Selecciona una imagen desde el ordenador, abre la galería del celular o arrástrala al área de carga. La imagen se guarda en el backend y la app muestra la URL permanente devuelta por el servidor.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="bg-neutral-950 p-6 rounded-2xl border border-neutral-800 h-fit space-y-5">
          <div className="flex items-center gap-2 border-b border-neutral-900 pb-3">
            ...
            <h3 className="text-xs font-bold uppercase tracking-wider text-white">Subir imagen real al servidor</h3>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              ...
              <span className="text-xs text-neutral-500 mt-1 max-w-xs">
                JPG, PNG o WEBP. No se usará una vista previa temporal: se enviará el archivo real al backend.
              </span>
              {selectedFile && (
                <button
                  type="button"
            ...
            <button type="submit" disabled={isSubmitting} className="w-full bg-red-600 hover:bg-red-500 disabled:bg-neutral-800 text-white font-bold text-xs p-3 rounded-xl transition-colors cursor-pointer mt-2 flex items-center justify-center gap-2">
              ...
              {isSubmitting ? 'Subiendo al servidor...' : 'Subir y guardar en servidor'}
            </button>
          </form>
          ...
          <div className="bg-neutral-900/40 p-3 rounded-xl border border-neutral-900 text-[11px] text-neutral-400 flex items-start gap-2">
            ...
            <span>La base de datos guarda solo la referencia permanente de la imagen. El archivo queda en el backend en <strong>/uploads/progress</strong>.</span>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {photosList.length > 0 ? (
            ...
            <div className="bg-neutral-950 border border-neutral-850 rounded-2xl p-12 text-center">
              ...
              <p className="text-xs text-neutral-500 mt-1 max-w-sm mx-auto">Utiliza el panel lateral para subir el primer archivo real y guardarlo permanentemente en el servidor.</p>
            </div>
          )}
        </div>
      </div>
