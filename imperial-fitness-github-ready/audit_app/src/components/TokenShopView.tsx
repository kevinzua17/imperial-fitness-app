...
export const TokenShopView: React.FC<TokenShopViewProps> = ({ currentUser, onSwitchTab }) => {
  ...
  const loadRewards = () => {
    Promise.all([getRewardBalanceFromApi(), listRewardProductsFromApi(), listRewardEventsFromApi()])
      ...
      .catch(() => setMsg('No se pudo cargar recompensas desde la API.'));
  };
  ...
  const createProduct = (e: React.FormEvent) => {
    e.preventDefault();
    createRewardProductInApi({ title, description: 'Producto creado desde panel admin.', cost, stock })
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-6 animate-fade-in">
      <div className="rounded-2xl border border-neutral-800 bg-gradient-to-r from-neutral-950 via-neutral-900 to-black p-6">
        ...
        <p className="text-sm text-neutral-400 mt-2 max-w-2xl">Balance, eventos, productos y canjes vienen del backend.</p>
      </div>
      ...
      <button onClick={() => onSwitchTab('challenges')} className="bg-red-600 hover:bg-red-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl">Ver retos reales</button>
    </div>
  );
};