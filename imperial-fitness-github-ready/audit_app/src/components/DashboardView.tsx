...
import { RetentionAlertApi, generateRetentionAlertsInApi, listRetentionAlertsFromApi, resolveRetentionAlertInApi } from '../services/retentionService';
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  useEffect(() => {
    getLogoFromApi().then(setGymLogoUrl).catch(() => undefined);
    getStatsSummaryFromApi().then(setStats).catch(() => setStats(null));
    if (currentUser.role !== 'client') {
      listRetentionAlertsFromApi().then(setRetentionAlerts).catch(() => setRetentionAlerts([]));
    }
  }, []);
  ...
  const handleSaveBrandingText = async () => {
    try {
      ...
      // Keep local preview if API is unavailable.
    }
  };
  ...
  const handleGenerateRetentionAlerts = async () => {
    try {
      ...
      setRetentionMessage('No se pudo generar alertas. Revisa backend y permisos.');
    }
  };
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-8 animate-fade-in">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-xs font-semibold text-neutral-400 tracking-wider uppercase">
                  Sede Central &bull; Sistema Conectado
                </span>
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              </div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                {gymName} <span className="text-red-500 font-light">| Ecosistema</span>
              </h1>
        ...
        {isEditingBranding && currentUser.role === 'admin' && (
          <div className="mt-4 pt-4 border-t border-neutral-800 grid grid-cols-1 md:grid-cols-2 gap-4 bg-black/40 p-4 rounded-xl animate-fade-in">
            ...
            <div>
              ...
              >
                {isUploadingLogo ? 'Subiendo logo al servidor...' : 'Seleccionar desde ordenador o galería'}
              </button>
            </div>
            <div>
              ...
              <button
                ...
                className="w-full bg-neutral-950 border border-neutral-700 rounded-lg px-3 py-2 text-xs text-white hover:border-red-600 transition-colors text-left disabled:opacity-50"
              >
                {isUploadingBackground ? 'Subiendo fondo al servidor...' : 'Cambiar imagen del login'}
              </button>
        ...
        {currentUser.role === 'client' && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-neutral-800/80">
            <div>
              ...
              <span className="text-lg font-bold text-white">{currentUser.bodyFat || 20}%</span>
              <div className="w-full bg-neutral-800 h-1 rounded-full mt-1">
                <div className="bg-red-500 h-1 rounded-full" style={{ width: `${currentUser.bodyFat || 20}%` }} />
              </div>
            </div>
            <div>
              <span className="text-[10px] text-neutral-500 uppercase block font-medium">Masa Muscular</span>
              <span className="text-lg font-bold text-white">{currentUser.muscleMass || 35} kg</span>
              <div className="w-full bg-neutral-800 h-1 rounded-full mt-1">
                <div className="bg-emerald-500 h-1 rounded-full" style={{ width: '65%' }} />
              </div>
            </div>
            <div>
              <span className="text-[10px] text-neutral-500 uppercase block font-medium">Agua Corporal</span>
              <span className="text-lg font-bold text-white">{currentUser.waterPercent || 55}%</span>
              <div className="w-full bg-neutral-800 h-1 rounded-full mt-1">
                <div className="bg-sky-500 h-1 rounded-full" style={{ width: `${currentUser.waterPercent || 55}%` }} />
              </div>
            </div>
            <div>
      ...
      <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-850 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-xs font-bold text-white block">Mapeo Completo del Ecosistema</span>
      ...
      {currentUser.role === 'admin' && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Módulo de Retención Especializada */}
            <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5 flex flex-col justify-between">
              <div>
                ...
                <button onClick={handleGenerateRetentionAlerts} className="w-full mb-3 rounded-lg bg-neutral-900 hover:bg-neutral-800 border border-neutral-700 px-3 py-2 text-xs font-bold text-white">
                  Generar alertas reales
                </button>
                {retentionMessage && <p className="text-[11px] text-neutral-400 mb-3">{retentionMessage}</p>}
                ...
                <div key={client.id} className="bg-neutral-900/60 border border-neutral-800 rounded-xl p-4 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      <img src={client.avatar} alt={client.name} className="w-8 h-8 rounded-full object-cover border border-neutral-700" />
                      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-5">
              ...
              {myRoutine ? (
                <div>
                  <h4 className="text-sm font-bold text-white mb-1">{myRoutine.title}</h4>
                  <p className="text-xs text-neutral-400 mb-4 font-light">{myRoutine.objective}</p>

                  <div className="space-y-3">
                    {myRoutine.days.slice(0, 2).map((d, idx) => (
                      <div key={idx} className="bg-neutral-900/60 border border-neutral-800 p-3 rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-xs font-bold text-red-400">{d.day}</span>
                          <span className="text-[10px] text-neutral-400">Enfoque: {d.focus}</span>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {d.exercises.map((ex, exIdx) => (
                            <div key={exIdx} className="bg-neutral-950 p-2 rounded border border-neutral-800 text-xs flex flex-col justify-between">
                              <span className="font-semibold text-white block text-[11px]">{ex.name}</span>
                              <span className="text-[10px] text-neutral-400 block font-mono mt-1">
                                {ex.sets} series &times; {ex.reps.split(' ')[0]}
                              </span>
                              {ex.notes && <span className="text-[9px] text-neutral-500 italic mt-0.5 block line-clamp-1">&bull; {ex.notes}</span>}
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-neutral-900">
                    <span className="text-[11px] text-neutral-400 italic block leading-relaxed">
                      <strong className="text-white not-italic font-semibold">Instrucción del Entrenador:</strong> "{myRoutine.specialistAdvice}"
                    </span>
                    <button
                      onClick={() => onNavigateTab('personal_plan')}
                      className="mt-3 text-xs text-red-500 hover:underline font-semibold block text-center w-full"
                    >
                      Ver plan completo de gimnasio &rarr;
                    </button>
                  </div>
                </div>
              ) : (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-neutral-950 border border-neutral-800 rounded-xl p-4">
              ...
              <div className="w-full bg-neutral-900 h-1.5 rounded-full overflow-hidden">
                <div className="bg-red-500 h-1.5 rounded-full" style={{ width: '65%' }} />
              </div>
              <button
