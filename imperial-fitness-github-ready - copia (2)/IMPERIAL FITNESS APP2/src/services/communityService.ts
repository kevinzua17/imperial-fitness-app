...
export type PostVisibility = 'public' | 'friends' | 'trainer' | 'private';
...
function mapPost(post: ApiCommunityPost): SocialPost {
  return {
    ...
    authorAvatar:
      absoluteUrl(post.author_avatar_url) ||
    ...
    visibilityLabel: post.visibility_label || 'Comunidad',
...
export async function listCommunityPostsFromApi(
  tag?: string,
  onlyFriends = false,
...
export async function createCommunityPostInApi(payload: {
  content: string;
  tags: string;
  visibility?: PostVisibility;
  file?: File | null;
