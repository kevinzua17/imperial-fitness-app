...
function absoluteUrl(url: string): string {
  ...
  if (url.startsWith('/foods/')) return url;
  return `${API_URL}${url}`;
}
...
export async function uploadExerciseImageToApi(exerciseId: string, file: File): Promise<string> {
  const formData = new FormData();
  formData.append('file', file);
  const response = await apiRequest<{ exercise_id: string; image_url: string }>(`/media/exercises/${encodeURIComponent(exerciseId)}/image`, {
    method: 'POST',
    body: formData,
  });
  return absoluteUrl(response.image_url);
}

export interface FoodImageOverridesResponse {
  images: Record<string, string>;
}

export async function getFoodImageOverridesFromApi(): Promise<Record<string, string>> {
  const response = await apiRequest<FoodImageOverridesResponse>('/media/foods/images');
  const images: Record<string, string> = {};
  Object.entries(response.images || {}).forEach(([key, value]) => {
    images[key] = absoluteUrl(value);
  });
  return images;
}

export async function saveFoodImageOverrideToApi(foodKey: string, imageUrl: string): Promise<string> {
  const response = await apiRequest<{ food_key: string; image_url: string }>(`/media/foods/${encodeURIComponent(foodKey)}/image`, {
    method: 'PATCH',
...
export async function uploadFoodImageToApi(foodKey: string, file: File): Promise<string> {
  const formData = new FormData();
  formData.append('file', file);
  const response = await apiRequest<{ food_key: string; image_url: string }>(`/media/foods/${encodeURIComponent(foodKey)}/image`, {
    method: 'POST',
