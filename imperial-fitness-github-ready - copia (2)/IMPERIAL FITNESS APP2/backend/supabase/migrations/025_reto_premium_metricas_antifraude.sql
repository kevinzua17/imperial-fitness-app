-- Reto Camino Imperial Premium: creación robusta de retos, acceso diferencial y métricas antifraude.
-- Ejecutar una sola vez después de 024_reto_camino_imperial_8_semanas.sql.
-- Si el reto no se estaba creando, este archivo deja tablas/columnas e índices completos.
...
alter table public.challenges
  ...
  add column if not exists refund_terms text default 'La garantía aplica si el participante cumple mínimo el 80% del reto y no mejora en al menos 2 indicadores medibles.',
  add column if not exists config_json text default '{"training_days_per_week":3,"exclusive_enabled":true,"weekly_review_day":"Domingo","anti_fraud_review":true}',
...
set challenge_type = coalesce(challenge_type, 'transformation_8w'),
    ...
    refund_terms = coalesce(refund_terms, 'La garantía aplica si el participante cumple mínimo el 80% del reto y no mejora en al menos 2 indicadores medibles.'),
    config_json = coalesce(config_json, '{"training_days_per_week":3,"exclusive_enabled":true,"weekly_review_day":"Domingo","anti_fraud_review":true}'),
...
-- Índices de lectura para revisión semanal del reto con hasta 500 usuarios.
